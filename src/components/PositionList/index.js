...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name wakandaswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name wakandaswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name wakandaswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  wakandaswap_RETURN: 'wakandaswap_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.wakandaswap_RETURN) {
          return p0?.wakandaswap?.return > p1?.wakandaswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.wakandaswap_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.wakandaswap_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.wakandaswap_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
